...
function expandDayTimes(dayTimes, dureeMinutes){
  ...
  dayTimes.forEach(dt=>{
    if(dt.timeEnd){
      ...
      let endMin = eh*60+em;
      if(endMin <= startMin) endMin += 24*60; // la plage traverse minuit (ex. 22h → 01h)
      while(startMin + dureeMinutes <= endMin){
        ...
        expanded.push({weekday: dt.weekday, time: `${hh}:${mm}`, dayOffset});
        startMin += dureeMinutes;
      }
    } else {
      expanded.push({weekday: dt.weekday, time: dt.time, dayOffset: 0});
    }
  });
  return expanded;
}
...
function generateWeeklyDates(startDateStr, dayTimes, weeks, dureeMinutes){
  ...
  expanded.forEach(dt=>{
    ...
    base.setDate(base.getDate() + (dt.dayOffset||0));
...
function listenPresence(){
  ...
  presenceUnsub = db.collection('presence').doc('teacher').onSnapshot(snap=>{
...
function startStudentMessagesListener(){
  ...
  studentMsgUnsub = db.collection('messages').where('studentUid','==',student.uid).orderBy('ts','asc')
    .onSnapshot(snap=>{
      const msgs = snap.docs.map(d=>({id:d.id, ...d.data()}));
      studentThreadMessages = msgs;
      studentUnreadCount = msgs.filter(m=>m.senderIsTeacher && !m.readByStudent).length;
      if(screen==='messages'){
        msgs.filter(m=>m.senderIsTeacher && !m.readByStudent).forEach(m=>{
          db.collection('messages').doc(m.id).update({readByStudent:true}).catch(()=>{});
        });
      }
      renderSidebar();
      if(screen==='messages') renderMessagesBody();
...
async function sendStudentMessage(){
  ...
  try{
    await db.collection('messages').add({
      studentUid: student.uid, studentName: `${student.prenom} ${student.nom}`,
      senderUid: student.uid, senderIsTeacher: false,
      text, readByTeacher: false, readByStudent: true,
      ts: firebase.firestore.FieldValue.serverTimestamp()
    });
  }catch(e){ alert("Impossible d'envoyer le message pour le moment."); }
}
...
function startTeacherMessagesListener(){
  if(teacherMsgUnsub) return;
  teacherMsgUnsub = db.collection('messages').orderBy('ts','asc').onSnapshot(snap=>{
    const byStudent = {};
    const toMarkRead = [];
    snap.docs.forEach(d=>{
      const m = d.data();
...
function teacherUnreadTotal(){ return teacherMsgThreads.reduce((s,t)=>s+t.unread,0); }
function loadTeacherMessages(){ startTeacherMessagesListener(); renderTeacherMessages(); }
function openTeacherThread(uid){
  teacherOpenThreadUid = uid;
  const t = teacherMsgThreads.find(x=>x.studentUid===uid);
  teacherOpenThreadName = t ? t.studentName : 'Élève';
  renderTeacherMessages();
}
function closeTeacherThread(){ teacherOpenThreadUid = null; renderTeacherMessages(); }
function renderTeacherMessages(){
  ...
  if(teacherMsgThreads.length===0){
    body.innerHTML = `<p class="teacher-empty">Aucun message pour le moment.</p>`;
    return;
  }
  body.innerHTML = teacherMsgThreads.map(t=>`
...
async function sendTeacherMessage(){
  ...
  if(!input) return;
  const text = input.value.trim();
  if(!text || !teacherOpenThreadUid) return;
  input.value = '';
  try{
    await db.collection('messages').add({
      studentUid: teacherOpenThreadUid, studentName: teacherOpenThreadName,
      senderUid: auth.currentUser.uid, senderIsTeacher: true,
      text, readByTeacher: true, readByStudent: false,
      ts: firebase.firestore.FieldValue.serverTimestamp()
    });
  }catch(e){ alert("Impossible d'envoyer le message pour le moment."); }
}
