...
(function(){
  ...
  /* Aiguilleur : si une fiche élève est ouverte (adminOpenStudentId), on
     affiche sa fiche détaillée (Dossiers & bilans / Profil / Forfait) ;
     sinon on retombe sur la liste riche définie dans 07-espace-professeure.js
     (paiement, forfait, bilan rapide). Voir la note dans ce dernier fichier :
     c'est volontairement le SEUL endroit qui définit ce nom de fonction. */
  window.renderTeacherStudentsList=function(){
    const body=document.getElementById('teacher-body');if(!body)return;
    if(adminOpenStudentId){
      const s=studentsData.find(x=>x.id===adminOpenStudentId);
      if(s){renderAdminStudent(s);return;}
      adminOpenStudentId=null;
    }
    renderStudentsFlatList();
  };
